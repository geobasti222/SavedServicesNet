...

    {
        ...
        public List<EPuntosInfo> ListDetallePuntosEmpresarias(string Nit)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    m = Factory.GetDetallePuntosEmpresaria(dr);
                    col.Add(m);
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Trace.WriteLine(string.Format("Glod Error: {0} , NameSpace: {1}, Clase: {2}, Metodo: {3} ", ex.Message, MethodBase.GetCurrentMethod().DeclaringType.Namespace, MethodBase.GetCurrentMethod().DeclaringType.Name, MethodBase.GetCurrentMethod().Name));
